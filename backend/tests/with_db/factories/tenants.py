...
class TenantFactory(factory.django.DjangoModelFactory):
    ...
    # Schema name baseado no slug, seguindo padrões PostgreSQL
    schema_name = factory.LazyAttribute(
        lambda obj: f"tenant_{obj.slug.replace('-', '_')}"[:63]  # Limite PostgreSQL
    )

    # Domain URL no formato do django-tenants
    domain_url = factory.LazyAttribute(lambda obj: f"{obj.slug}.wbjj.com")
