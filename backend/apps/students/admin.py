...
class AttendanceInline(TabularInline):
    ...
    extra = 0
    readonly_fields = ['created_at']
    fields = ['class_date', 'check_in_time', 'check_out_time', 'class_type', 'instructor']


@admin.register(Student)
class StudentAdmin(ModelAdmin):
    list_display = ['full_name', 'registration_number', 'get_belt_display', 'status', 'enrollment_date']
...
class GraduationAdmin(ModelAdmin):
    list_display = ['student', 'get_from_belt', 'get_to_belt', 'graduation_date', 'instructor']
    ...
    fieldsets = (
        ('Graduação', {
            'fields': ('student', 'from_belt', 'to_belt', 'graduation_date', 'instructor')
        }),
        ('Observações', {
            'fields': ('notes',)
        }),
...
class AttendanceAdmin(ModelAdmin):
    list_display = ['student', 'class_date', 'check_in_time', 'check_out_time', 'get_class_type_display']
