"""
Base classes para testes do sistema wBJJ MVP

Implementação MVP com filtro por tenant_id:
- SEMPRE usar TenantTestCase para isolamento por filtro
...
class BaseTenantTestCase(TestCase):
    """
    Classe base para testes que requerem isolamento de tenant MVP

    Implementação simplificada com filtro por tenant_id:
    - Isolamento por filtro de dados
    ...
    def setUp(self):
        """
        Setup padrão para testes com tenant MVP
        """
        super().setUp()
        cache.clear()
    ...
    def tearDown(self):
        """
        Cleanup padrão para testes com tenant MVP
        """
        cache.clear()
        super().tearDown()

    # Método run não é mais necessário no MVP (sem schemas separados)
