...
Implementação MVP com filtro por tenant_id:
...
- SEMPRE garantir isolamento de tenant por filtro
"""
...
class TenantViewSet(viewsets.ModelViewSet):
    """
    ViewSet base com isolamento de tenant por filtro
    ...
    def get_queryset(self):
        """
        Filtra queryset por tenant_id para isolamento MVP

        Implementação simplificada que filtra por tenant_id
        baseado no tenant detectado pelo middleware.
        """
        queryset = super().get_queryset()

        # Verificar se request tem tenant (detectado pelo middleware)
        if not hasattr(self.request, 'tenant') or not self.request.tenant:
            return queryset.none()

        # Filtrar por tenant_id se o model tem TenantMixin
    ...
    def perform_create(self, serializer):
        """
        Adiciona tenant automaticamente ao criar objetos
        """
        if not hasattr(self.request, 'tenant') or not self.request.tenant:
            raise ValidationError("Tenant é obrigatório")

        # Adicionar tenant se o model tem TenantMixin
